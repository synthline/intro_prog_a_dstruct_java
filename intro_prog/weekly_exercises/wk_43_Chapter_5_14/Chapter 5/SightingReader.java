...
public class SightingReader {
    ...
    private static final int SPOTTER = 0, ANIMAL = 1, COUNT = 2, AREA = 3, PERIOD = 4;
    ...
    public ArrayList<Sighting> getSightings(String filename) {
        ...
        Function<String, Sighting> createSighting = record -> {
        ...
        try {
            sightings = Files.lines(Paths.get(filename))
                    .filter(record -> record.length() > 0 && record.charAt(0) != '#').map(createSighting)
                    .filter(sighting -> sighting != null).collect(Collectors.toCollection(ArrayList::new));
        } catch (IOException e) {
